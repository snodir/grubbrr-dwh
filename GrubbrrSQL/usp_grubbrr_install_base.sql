...
SELECT Distinct 
       dense_rank() OVER(ORDER BY kiosk_software_version) as row_num,
       kiosk_software_version 
...
AND scanners is NOT NULL;

SELECT ordinal_position, column_name
FROM information_schema.columns 
WHERE table_name = 'vw_grubbrrinstallbase'
...
WHERE 1=1
--AND scanners is NOT NULL
--AND organization_status = 'Live'
AND is_org_smart_upsells_enabled = True;

SELECT *
FROM dim.organization
WHERE is_smart_upsells_enabled = True;


SELECT replace(replace(replace((pos_provider::text), '[', ''), ']', ''), '"', '') as formatted, 
       dim.array_to_text(pos_provider) as fn_format, *
FROM dim.vw_grubbrrinstallbase as gb
WHERE 1=1
AND gb.organization_id = 'org-5cf80db5-7a28-4dcf-846b-8cdf5f362269'
--and not dim.is_valid_jsonb(kiosks)
--AND gb.organization_status = 'Live'
--and scanners is not null
--AND gb.location_status = 'Live'
--AND gb.kiosk_mode = 'Live'
--AND replace(replace(replace((pos_provider::text), '[', ''), ']', ''), '"', '')
ORDER BY last_login_time desc;

SELECT org.createdon as org_created_date, 
       loc.createdon as loc_created_date,
       gb.*
FROM dim.vw_grubbrrinstallbase as gb
LEFT JOIN dim.organization as org 
       ON gb.organization_id = org.id 
LEFT JOIN dim.organization as loc 
       ON gb.location_id = loc.id;


SELECT * FROM dim.organization

select '{"a":1,"b":2}' :: jsonb, 
        ARRAY(select jsonb_object_keys('{"a":1,"b":2}')),
        --'{"a":1,"b":2}' :: jsonb -> 0,
        'false' :: BOOLEAN as bool_value_false_str,
        'true' :: BOOLEAN as bool_value_true_str,
        '0' :: BOOLEAN as bool_value_0_str,
        '1' :: BOOLEAN as bool_value_1_str,
        0 :: BOOLEAN as bool_value_0_int,
        1 :: BOOLEAN as bool_value_1_int,
        pg_typeof(1)
...
--ADD COLUMN IF NOT EXISTS CONSTRAINT locationid_pkey PRIMARY KEY (locationid);
--ADD COLUMN IF NOT EXISTS pos_provider text,
--ADD COLUMN IF NOT EXISTS loyalty_provider text,
--ADD COLUMN IF NOT EXISTS payment_provider text
ADD COLUMN IF NOT EXISTS scanners text;
...
CREATE TABLE if not exists /*drop view dim.vw_grubbrrinstallbase*/ dim.vw_grubbrrinstallbase
(
...
ALTER TABLE dim.vw_grubbrrinstallbase
ADD COLUMN IF NOT EXISTS CONSTRAINT locationid_deviceid_pk PRIMARY KEY (location_id, kiosk_id);
